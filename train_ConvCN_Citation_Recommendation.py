...
def save_model(model, optimizer, dataset, model_name):
    ...
    torch.save(model.state_dict(), os.path.join(os.getcwd(), 'models', dataset + '_' + model_name + '-' + str(
        args.num_epochs) + 'epochs' + '.pth'))


def load_model(model, dataset, model_all_name):
    # 加载模型
    model.load_state_dict(torch.load(os.path.join(os.getcwd(), 'models', dataset + '_' + model_all_name + '-' + str(
        args.num_epochs) + 'epochs' + '.pth')))
...
parser.add_argument("--start", default=0, type=int, help="启动的epoch数，用于断点继续训练")
...
if args.start != 0:
    load_model(kge_model, args.name, args.model_name + args.fold + '-' + str(args.start) + 'epochs')
...
if args.do_train:
    ...
    optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
'''
init_step = args.start  # just for debugging (remove this line later)
# step = init_step
...
for epoch in range(init_step, args.num_epochs):
    ...
    print('finish epoch ', epoch + 1, ' in ', intervaltime, ' mins or ', intervaltime / 60.0,
          ' hour(s), and finish at ', time.strftime('%H:%M:%S', finishhrs))
    ...
    # if (epoch+1)%3==0:
    #    save_model(kge_model, optimizer, args.name, args.model_name)
    if (epoch + 1) % 125 == 0:  # and (epoch+1)%2 == 0:
